...
const OfMagasin = props => {
    ...
    useEffect(() => {
        getOfsByEtat(userToken, "SendToMagasin").then((result) => {
            setOfList(result);
        }).catch((err) => {
            console.log(err);
        });
    ...
    const changeStateOf = (item) => {
        updateStateOf(userToken, { etat: "SendToMagasin", idOf: item.trackOf.idOf }).then((resp) => {
            console.log(resp)
        })
    }
    return (
